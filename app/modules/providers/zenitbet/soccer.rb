      ...
      def parse(page)
        page.search("div.b-league[id^=lid]").each do |league_node|
          ...
          league_node.search('tr[id^=gid]').each do |event_node|
            ...
            # first team win
            create_or_update_bet(bookmaker_event, 0, '1', nil, _1)
            # draw
            create_or_update_bet(bookmaker_event, 0, 'X', nil, _x)
            # second team win
            create_or_update_bet(bookmaker_event, 0, '2', nil, _2)
            # first team double chance
            create_or_update_bet(bookmaker_event, 0, '1X', nil, _1x)
            # first or second double chance
            create_or_update_bet(bookmaker_event, 0, '12', nil, _12)
            # second team double chance
            create_or_update_bet(bookmaker_event, 0, 'X2', nil, _x2)
            # totals
                ...
                if line.content =~ /Голы: /
                  ...
                  parts.each do |part|
                    [team_1, team_2].each_with_index do |team, i|
                      ...
                      if team_score
                        create_or_update_bet(bookmaker_event, 0, "I#{i+1}TO", 0.5, team_score[1].gsub(/,/, '.')) if team_score[1]
                        create_or_update_bet(bookmaker_event, 0, "I#{i+1}TU", 0.5, team_score[2].gsub(/,/, '.')) if team_score[2]
                      end
                    end
                    bts_y = part.match(/обе забьют - ([\d\.\,]+)/)
                    create_or_update_bet(bookmaker_event, 0, 'BTS_Y', nil, bts_y[1].gsub(/,/, '.')) if bts_y

                    bts_n = part.match(/хотя бы одна не забьёт - ([\d\.\,]+)/)
                    create_or_update_bet(bookmaker_event, 0, 'BTS_N', nil, bts_n[1].gsub(/,/, '.')) if bts_n

                    total_score = part.match(/нет голов - ([\d\.\,]+)/)
                    create_or_update_bet(bookmaker_event, 0, 'TU', 0.5, total_score[1].gsub(/,/, '.')) if total_score
                  end
                end
